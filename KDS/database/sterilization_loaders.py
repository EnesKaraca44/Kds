...
def load_sterilization_data(start_date_str, end_date_str):
    ...
    (
        ...
        WHERE KytTrh BETWEEN ? AND ? -- Parametre 1 ve 2
          AND Striptal = 0
        GROUP BY 
            OdaID,
            CAST(KytTrh AS DATE)
    ...
    INNER JOIN TblODALAR O WITH (NOLOCK)
        ON O.ODA_ID = S.OdaId
    WHERE S.CksTrh BETWEEN ? AND ? -- Parametre 3 ve 4 (Hata Buradaydı)
      AND S.CksSrvId = 37
    ...
    try:
        ...
        with st.spinner("Sterilizasyon verileri analiz ediliyor..."):
        ...
        st.error(f"❌ Sterilizasyon verisi yüklenirken hata: {e}")
        return pd.DataFrame()
    finally:
        conn.close()